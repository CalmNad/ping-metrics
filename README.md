...
Этот порт декларирует, что хранилище которое приложение готово использовать должно предоставить два метода, и описывает что story будет передавать на вход этих методов и что ожидает получить на выходе..

В результате, конкретная реализация хранилища будет определяться на этапе запуска (bootstrap) приложения. В конечном итоге story, которая использует хранилище с таким port может "под капотом" получить postgres, in momory, или mock хранилище.
...
Это уже конкретная реализация ports, объявленных в application. Именно тут принимаются решения об используемых библиотеках и фреймворках. И именно подмена адаптера позволяет, не трогая другие части приложения, изменить используемую БД, подменять реальные сервисы тестовыми имитациями и т.п.

Пример адаптера для http запросов можно мосмотреть [тут](../blob/master/server/src/adapters/controllers/http/index.ts).
